...
(cl:defmethod cl:initialize-instance :after ((m <Point32>) cl:&rest args)
  ...
  (cl:unless (cl:typep m 'Point32)
    (roslisp-msg-protocol:msg-deprecation-warning "using old message class name geometry_msgs-msg:<Point32> is deprecated: use geometry_msgs-msg:Point32 instead.")))
...
(cl:defmethod roslisp-msg-protocol:message-definition ((type (cl:eql '<Point32>)))
  ...
  (cl:format cl:nil "# This contains the position of a point in free space(with 32 bits of precision).~%# It is recommeded to use Point wherever possible instead of Point32.  ~%# ~%# This recommendation is to promote interoperability.  ~%#~%# This message is designed to take up less space when sending~%# lots of points at once, as in the case of a PointCloud.  ~%~%float32 x~%float32 y~%float32 z~%~%"))
(cl:defmethod roslisp-msg-protocol:message-definition ((type (cl:eql 'Point32)))
  "Returns full string definition for message of type 'Point32"
  (cl:format cl:nil "# This contains the position of a point in free space(with 32 bits of precision).~%# It is recommeded to use Point wherever possible instead of Point32.  ~%# ~%# This recommendation is to promote interoperability.  ~%#~%# This message is designed to take up less space when sending~%# lots of points at once, as in the case of a PointCloud.  ~%~%float32 x~%float32 y~%float32 z~%~%"))
