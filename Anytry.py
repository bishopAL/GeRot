...
while 1:
    ...
    cv2.imshow('f',img)
    f1 = img[:,:640]
    f1 = cv2.resize(f1,(240,320),interpolation=cv2.INTER_LINEAR)
    f2 = img[:,640:]
    f2 = cv2.resize(f2,(240,320),interpolation=cv2.INTER_LINEAR)
    cv2.imshow('f1',f1)
    cv2.imshow('f2',f2)
    ...
    if flag == 1:
        ...
        ufl = LgmdCal(src00,src01,src02)
        ufl_list.append(ufl)
        ufr = LgmdCal(src10,src11,src12)
        ufr_list.append(ufr)
